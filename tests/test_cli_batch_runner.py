...
def test_batch_run_targets() -> None:
	commands = [ sys.executable, 'videokit.py', 'batch-run', '--jobs-path', get_test_jobs_directory(), '--processors', 'region_debugger', '-t', get_test_example_file('target-240p-batch-*.jpg'), '-o', get_test_output_file('test-batch-run-targets-{index}.jpg') ]
...
def test_batch_run_sources_to_targets() -> None:
	commands = [ sys.executable, 'videokit.py', 'batch-run', '--jobs-path', get_test_jobs_directory(), '-s', get_test_example_file('target-240p-batch-*.jpg'), '-t', get_test_example_file('target-240p-batch-*.jpg'), '-o', get_test_output_file('test-batch-run-sources-to-targets-{index}.jpg') ]
