...
from videokit.app_context import detect_app_context
from videokit.common_helper import is_windows
from videokit.execution import create_inference_providers, has_execution_provider
from videokit.exit_helper import fatal_exit
from videokit.filesystem import get_file_name, is_file
from videokit.time_helper import calculate_end_time
from videokit.types import DownloadSet, ExecutionProvider, InferencePool, InferencePoolSet
...
def get_inference_context(module_name : str, model_names : List[str], execution_device_id : int, execution_providers : List[ExecutionProvider]) -> str:
	model_names_filtered = [m for m in model_names if m is not None]
	inference_context = '.'.join([ module_name ] + model_names_filtered + [ str(execution_device_id) ] + list(execution_providers))
