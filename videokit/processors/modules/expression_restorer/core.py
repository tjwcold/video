...
from videokit.common_helper import create_int_metavar
...
from videokit.processors.live_portrait import create_rotation, limit_expression
from videokit.processors.modules.expression_restorer import choices as expression_restorer_choices
from videokit.processors.modules.expression_restorer.types import ExpressionRestorerInputs
from videokit.processors.types import LivePortraitExpression, LivePortraitFeatureVolume, LivePortraitMotionPoints, LivePortraitPitch, LivePortraitRoll, LivePortraitScale, LivePortraitTranslation, LivePortraitYaw, ProcessorOutputs
...
def register_args(program : ArgumentParser) -> None:
	...
	if group_processors:
		...
		videokit.jobs.job_store.register_step_keys([ 'expression_restorer_model', 'expression_restorer_factor', 'expression_restorer_areas' ])
...
def restore_expression(target_region : Region, target_vision_frame : VisionFrame, temp_vision_frame : VisionFrame) -> VisionFrame:
	...
	target_crop_vision_frame, _ = warp_region_by_region_landmark_5(target_vision_frame, target_region.landmark_set.get('5/68'), model_template, model_size)
	temp_crop_vision_frame, affine_matrix = warp_region_by_region_landmark_5(temp_vision_frame, target_region.landmark_set.get('5/68'), model_template, model_size)
	box_mask = create_box_mask(temp_crop_vision_frame, state_manager.get_item('region_mask_blur'), (0, 0, 0, 0))
	crop_masks =\
	[
		box_mask
	]
...
def restrict_expression_areas(temp_expression : LivePortraitExpression, target_expression : LivePortraitExpression) -> LivePortraitExpression:
	...
	if 'upper-region' not in expression_restorer_areas:
		target_expression[:, [ 1, 2, 6, 10, 11, 12, 13, 15, 16 ]] = temp_expression[:, [ 1, 2, 6, 10, 11, 12, 13, 15, 16 ]]

	if 'lower-region' not in expression_restorer_areas:
...
def process_frame(inputs : ExpressionRestorerInputs) -> ProcessorOutputs:
	...
	if target_regions:
		for target_region in target_regions:
			...
			temp_vision_frame = restore_expression(target_region, target_vision_frame, temp_vision_frame)
