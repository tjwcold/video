...
from videokit.types import AudioEncoder, Fps, Scale, VideoEncoder, VideoPreset
from videokit.uis.core import get_ui_components, register_ui_component
...
def render() -> None:
	...
	OUTPUT_IMAGE_QUALITY_SLIDER = gradio.Slider(
		...
		step = calculate_int_step(videokit.choices.output_image_quality_range),
		minimum = videokit.choices.output_image_quality_range[0],
		maximum = videokit.choices.output_image_quality_range[-1],
	...
	OUTPUT_IMAGE_SCALE_SLIDER = gradio.Slider(
		...
		step = calculate_float_step(videokit.choices.output_image_scale_range),
		value = state_manager.get_item('output_image_scale'),
		minimum = videokit.choices.output_image_scale_range[0],
		maximum = videokit.choices.output_image_scale_range[-1],
		visible = is_image(state_manager.get_item('target_path'))
	)
	...
	OUTPUT_AUDIO_QUALITY_SLIDER = gradio.Slider(
		...
		step = calculate_int_step(videokit.choices.output_audio_quality_range),
		minimum = videokit.choices.output_audio_quality_range[0],
		maximum = videokit.choices.output_audio_quality_range[-1],
		visible = is_video(state_manager.get_item('target_path'))
	)
	OUTPUT_AUDIO_VOLUME_SLIDER = gradio.Slider(
		...
		step = calculate_int_step(videokit.choices.output_audio_volume_range),
		minimum = videokit.choices.output_audio_volume_range[0],
		maximum = videokit.choices.output_audio_volume_range[-1],
		visible = is_video(state_manager.get_item('target_path'))
	)
	...
	OUTPUT_VIDEO_PRESET_DROPDOWN = gradio.Dropdown(
		...
		choices = videokit.choices.output_video_presets,
	...
	OUTPUT_VIDEO_QUALITY_SLIDER = gradio.Slider(
		...
		step = calculate_int_step(videokit.choices.output_video_quality_range),
		minimum = videokit.choices.output_video_quality_range[0],
		maximum = videokit.choices.output_video_quality_range[-1],
		visible = is_video(state_manager.get_item('target_path'))
	)
	OUTPUT_VIDEO_SCALE_SLIDER = gradio.Slider(
		label = translator.get('uis.output_video_scale_slider'),
		step = calculate_float_step(videokit.choices.output_video_scale_range),
		value = state_manager.get_item('output_video_scale'),
		minimum = videokit.choices.output_video_scale_range[0],
		maximum = videokit.choices.output_video_scale_range[-1],
		visible = is_video(state_manager.get_item('target_path'))
	)
