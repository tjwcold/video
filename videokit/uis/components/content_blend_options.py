...
def render() -> None:
	...
	has_deep_swapper = 'content_blend' in state_manager.get_item('processors')
	DEEP_SWAPPER_MODEL_DROPDOWN = gradio.Dropdown(
		label = translator.get('uis.model_dropdown', 'videokit.processors.modules.deep_swapper'),
	...
	DEEP_SWAPPER_MORPH_SLIDER = gradio.Slider(
		label = translator.get('uis.morph_slider', 'videokit.processors.modules.deep_swapper'),
		...
		visible = has_deep_swapper and load_processor_module('content_blend').get_inference_pool() and load_processor_module('content_blend').has_morph_input()
	)
...
def remote_update(processors : List[str]) -> Tuple[gradio.Dropdown, gradio.Slider]:
	has_deep_swapper = 'content_blend' in processors
	return gradio.Dropdown(visible = has_deep_swapper), gradio.Slider(visible = has_deep_swapper and load_processor_module('content_blend').get_inference_pool() and load_processor_module('content_blend').has_morph_input())


def update_deep_swapper_model(deep_swapper_model : ContentBlendModel) -> Tuple[gradio.Dropdown, gradio.Slider]:
	deep_swapper_module = load_processor_module('content_blend')
