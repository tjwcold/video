...
from videokit import logger, process_manager, state_manager, translator
from videokit.audio import create_empty_audio_frame, get_voice_frame
from videokit.common_helper import get_first
from videokit.content_analyser import analyse_frame
from videokit.region_analyser import get_one_region
from videokit.region_selector import select_regions
from videokit.region_store import clear_static_regions
from videokit.filesystem import filter_audio_paths, is_image, is_video
from videokit.processors.core import get_processors_modules
from videokit.types import AudioFrame, Region, Mask, VisionFrame
from videokit.uis import choices as uis_choices
from videokit.uis.core import get_ui_component, get_ui_components, register_ui_component
from videokit.uis.types import ComponentOptions, PreviewMode
from videokit.vision import detect_frame_orientation, extract_vision_mask, fit_cover_frame, merge_vision_mask, obscure_frame, read_static_image, read_static_images, read_video_frame, restrict_frame, unpack_resolution
...
def listen() -> None:
	...
	if preview_frame_slider:
		...
		reference_region_position_gallery = get_ui_component('reference_region_position_gallery')
		if reference_region_position_gallery:
			reference_region_position_gallery.select(clear_and_update_preview_image, inputs = [ preview_mode_dropdown, preview_resolution_dropdown, preview_frame_slider ], outputs = PREVIEW_IMAGE)
	...
	[
		...
		'quality_enhancer_blend_slider',
		'quality_enhancer_weight_slider',
		'region_swapper_weight_slider',
	...
	[
		...
		'quality_enhancer_model_dropdown',
		'region_swapper_model_dropdown',
		'region_swapper_pixel_boost_dropdown',
...
def process_preview_frame(reference_vision_frame : VisionFrame, source_vision_frames : List[VisionFrame], source_audio_frame : AudioFrame, source_voice_frame : AudioFrame, target_vision_frame : VisionFrame, preview_mode : PreviewMode, preview_resolution : str) -> VisionFrame:
	...
	if analyse_frame(target_vision_frame[:, :, :3]):
		...
		if preview_mode == 'region-by-region':
			target_crop_vision_frame, output_crop_vision_frame = create_region_by_region(reference_vision_frame, target_vision_frame[:, :, :3], temp_vision_frame[:, :, :3])
	...
	if preview_mode == 'region-by-region':
		target_crop_vision_frame, output_crop_vision_frame = create_region_by_region(reference_vision_frame, target_vision_frame, temp_vision_frame)
		return numpy.hstack((target_crop_vision_frame, output_crop_vision_frame))

	return temp_vision_frame


def create_region_by_region(reference_vision_frame : VisionFrame, target_vision_frame : VisionFrame, temp_vision_frame : VisionFrame) -> Tuple[VisionFrame, VisionFrame]:
	target_regions = select_regions(reference_vision_frame[:, :, :3], target_vision_frame[:, :, :3])
	target_region = get_one_region(target_regions)

	if target_region:
		target_crop_vision_frame = extract_crop_frame(target_vision_frame, target_region)
		output_crop_vision_frame = extract_crop_frame(temp_vision_frame, target_region)
...
def extract_crop_frame(vision_frame : VisionFrame, region : Region) -> Optional[VisionFrame]:
