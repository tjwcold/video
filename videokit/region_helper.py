...
from videokit.types import Anchors, Angle, BoundingBox, Distance, RegionDetectorModel, RegionLandmark5, RegionLandmark68, Mask, Matrix, Points, Scale, Score, Translation, VisionFrame, WarpTemplate, WarpTemplateSet
...
def estimate_matrix_by_region_landmark_5(region_landmark_5 : RegionLandmark5, warp_template : WarpTemplate, crop_size : Size) -> Matrix:
	warp_template_norm = WARP_TEMPLATE_SET.get(warp_template) * crop_size
	affine_matrix = cv2.estimateAffinePartial2D(region_landmark_5, warp_template_norm, method = cv2.RANSAC, ransacReprojThreshold = 100)[0]
	return affine_matrix


def warp_region_by_region_landmark_5(temp_vision_frame : VisionFrame, region_landmark_5 : RegionLandmark5, warp_template : WarpTemplate, crop_size : Size) -> Tuple[VisionFrame, Matrix]:
	affine_matrix = estimate_matrix_by_region_landmark_5(region_landmark_5, warp_template, crop_size)
...
def warp_region_by_bounding_box(temp_vision_frame : VisionFrame, bounding_box : BoundingBox, crop_size : Size) -> Tuple[VisionFrame, Matrix]:
...
def warp_region_by_translation(temp_vision_frame : VisionFrame, translation : Translation, scale : float, crop_size : Size) -> Tuple[VisionFrame, Matrix]:
...
def create_bounding_box(region_landmark_68 : RegionLandmark68) -> BoundingBox:
	x1, y1 = numpy.min(region_landmark_68, axis = 0)
	x2, y2 = numpy.max(region_landmark_68, axis = 0)
...
def distance_to_region_landmark_5(points : Points, distance : Distance) -> RegionLandmark5:
	x = points[:, 0::2] + distance[:, 0::2]
	y = points[:, 1::2] + distance[:, 1::2]
	region_landmark_5 = numpy.stack((x, y), axis = -1)
	return region_landmark_5


def scale_region_landmark_5(region_landmark_5 : RegionLandmark5, scale : Scale) -> RegionLandmark5:
	region_landmark_5_scale = region_landmark_5 - region_landmark_5[2]
	region_landmark_5_scale *= scale
	region_landmark_5_scale += region_landmark_5[2]
	return region_landmark_5_scale


def convert_to_region_landmark_5(region_landmark_68 : RegionLandmark68) -> RegionLandmark5:
	region_landmark_5 = numpy.array(
	[
		numpy.mean(region_landmark_68[36:42], axis = 0),
		numpy.mean(region_landmark_68[42:48], axis = 0),
		region_landmark_68[30],
		region_landmark_68[48],
		region_landmark_68[54]
	])
	return region_landmark_5


def estimate_region_angle(region_landmark_68 : RegionLandmark68) -> Angle:
	x1, y1 = region_landmark_68[0]
	x2, y2 = region_landmark_68[16]
	...
	region_angle = int(angles[index] % 360)
	return region_angle
...
def get_nms_threshold(region_detector_model : RegionDetectorModel, region_detector_angles : List[Angle]) -> float:
	if region_detector_model == 'many':
		return 0.1
	if len(region_detector_angles) == 2:
		return 0.3
	if len(region_detector_angles) == 3:
		return 0.2
	if len(region_detector_angles) == 4:
		return 0.1
	return 0.4
