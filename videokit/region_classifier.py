...
from videokit.types import Age, DownloadScope, RegionLandmark5, Gender, InferencePool, ModelOptions, ModelSet, Race, VisionFrame
...
def classify_region(temp_vision_frame : VisionFrame, region_landmark_5 : RegionLandmark5) -> Tuple[Gender, Age, Race]:
	model_template = get_model_options().get('template')
	model_size = get_model_options().get('size')
	model_mean = get_model_options().get('mean')
	model_standard_deviation = get_model_options().get('standard_deviation')
	crop_vision_frame, _ = warp_region_by_region_landmark_5(temp_vision_frame, region_landmark_5, model_template, model_size)
...
def forward(crop_vision_frame : VisionFrame) -> Tuple[List[int], List[int], List[int]]:
	region_classifier = get_inference_pool().get('region_classifier')

	with conditional_thread_semaphore():
		race_id, gender_id, age_id = region_classifier.run(None,
		{
