...
from videokit.types import DownloadScope, DownloadSet, RegionLandmark68, RegionMaskArea, RegionMaskRegion, InferencePool, Mask, ModelSet, Padding, VisionFrame
...
def collect_model_downloads() -> Tuple[DownloadSet, DownloadSet]:
	...
	for region_occluder_model in [ 'xseg_1', 'xseg_2', 'xseg_3' ]:
		if state_manager.get_item('region_occluder_model') in [ 'many', region_occluder_model ]:
			model_hash_set[region_occluder_model] = model_set.get(region_occluder_model).get('hashes').get('region_occluder')
			model_source_set[region_occluder_model] = model_set.get(region_occluder_model).get('sources').get('region_occluder')

	for region_parser_model in [ 'bisenet_resnet_18', 'bisenet_resnet_34' ]:
		if state_manager.get_item('region_parser_model') == region_parser_model:
			model_hash_set[region_parser_model] = model_set.get(region_parser_model).get('hashes').get('region_parser')
			model_source_set[region_parser_model] = model_set.get(region_parser_model).get('sources').get('region_parser')
...
def create_box_mask(crop_vision_frame : VisionFrame, region_mask_blur : float, region_mask_padding : Padding) -> Mask:
	crop_size = crop_vision_frame.shape[:2][::-1]
	blur_amount = int(crop_size[0] * 0.5 * region_mask_blur)
	blur_area = max(blur_amount // 2, 1)
	box_mask : Mask = numpy.ones(crop_size).astype(numpy.float32)
	box_mask[:max(blur_area, int(crop_size[1] * region_mask_padding[0] / 100)), :] = 0
	box_mask[-max(blur_area, int(crop_size[1] * region_mask_padding[2] / 100)):, :] = 0
	box_mask[:, :max(blur_area, int(crop_size[0] * region_mask_padding[3] / 100))] = 0
	box_mask[:, -max(blur_area, int(crop_size[0] * region_mask_padding[1] / 100)):] = 0
...
def create_occlusion_mask(crop_vision_frame : VisionFrame) -> Mask:
	...
	for model_name in model_names:
		...
		temp_mask = forward_occlude_region(prepare_vision_frame, model_name)
...
def create_area_mask(crop_vision_frame : VisionFrame, region_landmark_68 : RegionLandmark68, region_mask_areas : List[RegionMaskArea]) -> Mask:
	crop_size = crop_vision_frame.shape[:2][::-1]
	landmark_points = []

	for region_mask_area in region_mask_areas:
		if region_mask_area in videokit.choices.region_mask_area_set:
			landmark_points.extend(videokit.choices.region_mask_area_set.get(region_mask_area))

	convex_hull = cv2.convexHull(region_landmark_68[landmark_points].astype(numpy.int32))
...
def create_region_mask(crop_vision_frame : VisionFrame, region_mask_regions : List[RegionMaskRegion]) -> Mask:
	...
	region_mask = forward_parse_region(prepare_vision_frame)
	region_mask = numpy.isin(region_mask.argmax(0), [ videokit.choices.region_mask_region_set.get(region_mask_region) for region_mask_region in region_mask_regions ])
	region_mask = cv2.resize(region_mask.astype(numpy.float32), crop_vision_frame.shape[:2][::-1])
	region_mask = (cv2.GaussianBlur(region_mask.clip(0, 1), (0, 0), 5).clip(0.5, 1) - 0.5) * 2
	return region_mask


def forward_occlude_region(prepare_vision_frame : VisionFrame, model_name : str) -> Mask:
	region_occluder = get_inference_pool().get(model_name)

	with conditional_thread_semaphore():
		occlusion_mask : Mask = region_occluder.run(None,
		{
...
def forward_parse_region(prepare_vision_frame : VisionFrame) -> Mask:
	...
	with conditional_thread_semaphore():
		region_mask : Mask = region_parser.run(None,
		{
			'input': prepare_vision_frame
		})[0][0]
